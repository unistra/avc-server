...
<%
	...
	String formation = EncodageStr.toUTF8(request.getParameter("formation"));
...
<html>
<head>
<title>AudioVideoCours : cours par formation</title>
...
<%
		/* Affichage d'une métadonnée RSS pour la formation */
    ...
    <td align="center"><table width="757" border="0" cellspacing="0" cellpadding="0">
     ...
     <table width="100%">
			  <tr>
				...
				<%
					/* Sélection et affichage de tous les cours d'une formation */
				
					sqlbean1.setWriter(out);
					sqlbean1.connect();
					String queryinit = "SELECT * FROM \"AudioCours\" a, \"Media\" m WHERE a.\"AudioCoursId\" = m.\"AudioCoursId\"";
					queryinit += " AND \"Visible\" = 1 AND Initcap(\"Rating\") = Initcap(?) ORDER BY \"CreationDate\" DESC, a.\"AudioCoursId\" DESC LIMIT " + nbrCoursMax + " OFFSET " + start ;
					sqlbean1.pquery(queryinit,formation);
		   			   ...
		   			   <ul><li><small>
							...
							%>
						    	...
						    	<form method="post" action="verif_code.jsp">
						    		...
						    		<input type="hidden" name="page" value="cours_par_formation">
				...
				<%
					...
					querycount += " AND \"Visible\" = 1 AND Initcap(\"Rating\") = Initcap(?) ";
					sqlbean1.pquery(querycount,formation);
					...
					/* Lien première page */
					out.println("<a href=\"./cours_par_formation.jsp?formation=" + formation + "&start=0\">&lt;&lt;</a>&nbsp;");
					
					/* Affichage des liens avec les numéros de page*/
					for( int i=debut ; i<= fin ; i++)
						out.println((i != pageCourante ? "<a href=\"./cours_par_formation.jsp?formation=" + formation + "&start=" + ((i-1) * nbrCoursMax) + "\">" : "") + i + (i != pageCourante ? "</a>" : "") +"&nbsp;");
					
					/* Lien dernière page */
					out.println("<a href=\"./cours_par_formation.jsp?formation=" + formation + "&start=" + ((nbrPages-1) * nbrCoursMax) + "\">&gt;&gt;</a>&nbsp;");
